<template>
  <div class="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased pb-20">
    ...
    <div class="bg-[#D7070D] text-white text-xs py-2 px-4 text-center font-bold tracking-wider flex items-center justify-center gap-3 shadow-xs">
      ...
      <span class="hidden sm:inline">• GARANTIA ESCRITA DE 5 ANOS</span>
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <span class="text-[10px] font-black uppercase tracking-widest text-[#D7070D] block">
              PORTAL EXCLUSIVO DO PROPRIETÁRIO
            </span>
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
              {{ data?.lead?.name || 'Cliente' }} — Acompanhamento da Obra
            </h1>
            <p class="text-xs text-slate-500 font-mono mt-0.5">
              📍 {{ data?.lead?.address || data?.lead?.city || 'Massachusetts' }} • {{ data?.lead?.serviceInterested || 'Pintura & Reforma' }}
            </p>
          </div>
        </div>

        <div class="text-center sm:text-right">
          <span class="text-[10px] font-mono text-slate-400 block uppercase">Código do Projeto</span>
      ...
      <div v-else class="space-y-6">
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 space-y-6">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
            <div>
              <span class="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Progresso Atual</span>
              <h2 class="text-lg sm:text-xl font-black text-slate-900">
                Fase: <span class="text-[#D7070D]">{{ formatStageName(data?.lead?.status) }}</span>
              </h2>
            </div>
            <span class="text-xs font-mono font-bold px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 self-start sm:self-auto">
              Equipe Ativa em Massachusetts
            </span>
          </div>
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 space-y-4">
          <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <div class="flex items-center gap-2.5">
              ...
              <div>
                <h3 class="text-base font-black text-slate-900">Fotos e Registro de Execução</h3>
                <p class="text-xs text-slate-500">Imagens em tempo real registradas pela equipe técnica</p>
              </div>
            </div>
            <span class="text-xs font-mono text-slate-400">{{ data?.photos?.length || 2 }} imagens</span>
          </div>
            ...
            >
              ...
              <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-4 text-white">
                <span class="text-[10px] font-black uppercase text-red-400 tracking-wider">
                  {{ photo.type === 'PROGRESSO_OBRA' ? 'Progresso Diário' : 'Estado Inicial' }}
                </span>
                <p class="text-xs font-medium text-slate-100 line-clamp-2 mt-0.5">
                  {{ photo.aiAnalysis || 'Preparação de superfície, lixamento e isolamento de áreas adjacentes.' }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 space-y-4">
            <div class="flex items-center gap-2.5 border-b border-slate-100 pb-3">
              ...
              <h3 class="text-sm font-black text-slate-900 uppercase">Garantia e Termos Legais</h3>
            </div>

            <div class="space-y-3 text-xs text-slate-700">
              <div class="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-emerald-900 space-y-1">
                <span class="font-bold block">🛡️ Garantia Escrita de 5 Anos:</span>
                <p class="text-[11px] text-emerald-800 leading-relaxed">
                  Proteção completa contra descascamento precoce e falhas de adesão em superfícies preparadas com primer de alta aderência.
                </p>
              </div>

              <div class="space-y-1.5 pt-1">
                <div class="flex justify-between py-1 border-b border-slate-100">
                  <span class="text-slate-500">Contrato Legal:</span>
                  <span class="font-bold text-slate-800">{{ data?.contract?.contractNumber || 'HIC-2026-001' }}</span>
                </div>
                <div class="flex justify-between py-1 border-b border-slate-100">
                  <span class="text-slate-500">Registro Estadual:</span>
                  <span class="font-bold text-slate-800">MA HIC #204891</span>
                </div>
                <div class="flex justify-between py-1">
                  <span class="text-slate-500">Valor Total Contratado:</span>
          ...
          <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 space-y-4 flex flex-col justify-between">
            <div>
              <div class="flex items-center gap-2.5 border-b border-slate-100 pb-3">
                ...
                <h3 class="text-sm font-black text-slate-900 uppercase">Status do Pagamento</h3>
              </div>

              <div class="mt-4 space-y-3 text-xs">
                <div class="flex items-center justify-between p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
                  <div>
                    <span class="text-[10px] text-slate-500 font-bold uppercase block">Sinal de Entrada (Máx 1/3 Lei MA)</span>
                  ...
                  <span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase"
                    ...
                    {{ data?.contract?.signed ? '✓ PAGO VIA STRIPE' : 'AGUARDANDO' }}
                  </span>
                </div>

                <p class="text-[11px] text-slate-500 leading-relaxed">
                  Os pagamentos são processados com segurança bancária criptografada pela Stripe e estão em total conformidade com a legislação de Massachusetts.
                </p>
              </div>
            </div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative z-10">
            <div class="space-y-2 max-w-xl">
              <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-600/20 text-red-400 text-xs font-bold border border-red-500/30">
                <span>🎁</span>
                <span>Programa Oficial de Indicação Tony's</span>
              </div>
              <h3 class="text-xl sm:text-2xl font-black text-white tracking-tight">
                Indique um Vizinho ou Amigo e Ganhe <span class="text-red-500">$150 via Stripe</span>!
              </h3>
              <p class="text-xs text-slate-300 leading-relaxed">
                Seu amigo ganha <strong class="text-white">$100 de desconto de boas-vindas</strong> na pintura residencial e, assim que o contrato for assinado, você recebe <strong class="text-emerald-400">$150 de crédito no seu cartão</strong> ou desconto na sua próxima reforma.
              </p>
            </div>

            <div class="flex-shrink-0 bg-white/5 border border-white/10 p-4 rounded-2xl space-y-3 sm:w-80">
              <span class="text-[11px] font-bold uppercase text-slate-400 block tracking-wider">Seu Link de Indicação Exclusivo:</span>
              ...
              <span class="text-[10px] text-slate-400 block text-center">
                ✓ Payout automático via Stripe Instant Transfer
              </span>
            </div>
          </div>
        </div>
...
const stagesList = [
  { key: 'NOVO', title: '1. Orçamento', subtitle: 'Solicitação inicial' },
  { key: 'PROPOSTA', title: '2. Proposta', subtitle: 'Medição e escopo' },
  { key: 'CONVERTIDO', title: '3. Aprovado', subtitle: 'Contrato firmado' },
  { key: 'EM_EXECUCAO', title: '4. Em Obras', subtitle: 'Execução no local' },
  { key: 'FINALIZADO', title: '5. Concluído', subtitle: 'Vistoria e garantia' }
]
...
const samplePhotos = [
  {
    ...
    type: 'PROGRESSO_OBRA',
    aiAnalysis: 'Pintura externa em andamento: raspagem, primer a óleo e selagem elastomérica concluídas.'
  },
  {
    url: 'https://doorbridgefix.com/wp-content/uploads/2026/03/IMG_2237-scaled.jpg',
    type: 'FOTO_ORIGINAL',
    aiAnalysis: 'Armários de cozinha preparados e isolados para pintura em spray com acabamento acetinado.'
  }
]
...
async function fetchPortalData() {
  ...
  try {
    ...
    if (res?.success) {
      ...
      error.value = res?.message || 'Não foi possível carregar os dados.'
    }
  } catch (err) {
    console.error('Erro ao buscar portal do cliente:', err)
...
function formatStageName(status) {
  const names = {
    NOVO: 'Orçamento Inicial Recebido',
    EM_ATENDIMENTO: 'Em Atendimento Técnico',
    PROPOSTA: 'Proposta Comercial Emitida',
    CONVERTIDO: 'Contrato Firmado & Agendado',
    EM_EXECUCAO: 'Em Execução no Canteiro',
    FINALIZADO: 'Obra 100% Concluída'
  }
  return names[status] || 'Em Execução no Canteiro'
}
