...
export default defineEventHandler(async (event) => {
  try {
    ...
    let smsSent = false
    const smsMessage = `Hi ${clientName}, thank you for choosing First Boston Painters for your ${service}! If you had a great experience with our crew, could you take 30 seconds to leave us a quick 5-star Google review? It helps our local team immensely: ${googleReviewUrl}`
    ...
    if ((channel === 'EMAIL' || channel === 'BOTH') && lead.email) {
      const emailSubject = `How did our painting crew do? Review First Boston Painters on Google`
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #222; border: 1px solid #eee; border-radius: 12px; overflow: hidden;">
          <div style="background: #111; padding: 24px; text-align: center; border-bottom: 4px solid #ff7902;">
            <h2 style="color: #ff7902; margin: 0; font-size: 22px;">FIRST BOSTON PAINTERS & SERVICES</h2>
            <p style="color: #aaa; margin: 5px 0 0 0; font-size: 13px;">Licensed and Insured in Massachusetts since 2015</p>
          </div>
          <div style="padding: 30px 24px; background: #fff;">
            <p style="font-size: 16px;">Hi <strong>${clientName}</strong>,</p>
            <p style="font-size: 15px; line-height: 1.6;">Thank you for trusting First Boston Painters with your <strong>${service}</strong>! Our mission is to deliver flawless prep work and lasting beauty for your home.</p>
            <p style="font-size: 15px; line-height: 1.6;">Word-of-mouth and customer feedback are everything to our local painters. Could you take 30 seconds to share your experience on our Google Business Profile?</p>
            ...
            <p style="font-size: 14px; color: #555;">If there was anything less than perfect, please reply directly to this email so our management team can address it immediately.</p>
          </div>
          <div style="background: #f9f9f9; padding: 16px; text-align: center; font-size: 11px; color: #888;">
            First Boston Painters and Services Corp. • Boston & Greater Massachusetts • Licensed & Insured
          </div>
        </div>
      `
